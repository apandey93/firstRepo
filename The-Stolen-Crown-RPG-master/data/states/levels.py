...
import copy, sys
import pygame as pg
from .. import tools, collision, collisiondeep
...
class LevelState(tools._State):
    ...
    def startup(self, current_time, game_data, totaltime):
        ...
        self.collision_handler = collision.CollisionHandler(self.player,
                                                            ...
                                                            self.portals,
                                                            self)
	self.collisionlist= self.collision_handler.blocker_list()
            ...
            for object in self.renderer.tmx_data.getObjects():
                ...
                if properties['name'] == 'start point':
                    if last_state == properties['state']:
                        ...
                        player.rect.x = posx
                        player.rect.y = posy
			player.setPortalist(self.portals)
			player.setMapLevel(self.name)
			player.colitions(self.collisionlist)
	...
	self.player.colitions(self.collision_handler.blocker_list())
	...
	self.player.setPortalist(self.portals)
	self.player.setMapLevel(self.name)
	self.player.setTime(self.totaltime)
