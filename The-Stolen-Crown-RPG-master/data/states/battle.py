...
class Battle(tools._State):
    ...
    def startup(self, current_time, game_data,totaltime):
        """
        ...
        '''
        Additions made by Jacob Locik
        '''
        self.health_stats = game_data['player stats']['health']
        self.hp_after_potion = (self.health_stats['current'])
    ...
    def update(self, surface, keys, current_time,totaltime):
        """
    ...
    def check_input(self, keys):
        """
            Battle Agent that allows our in game character to battle enemies
            on its own.
        """
        #initialize values for current health and current maximum health
        self.current_health = (self.health_stats['current'])
        self.max_health = (self.health_stats['maximum']) 
        #Decides when the player should use a potion
        if self.hp_after_potion < (self.max_health * .2) and 'Healing Potion' in self.game_data['player inventory']:
            self.hp_after_potion += 30
        ...
        #Decides when the player should run away from a battle
        elif self.current_health < (self.max_health *.5) and 'Healing Potion' not in self.game_data['player inventory']:
            if self.state == c.SELECT_ACTION:
                self.notify(c.CLICK2)
                self.try_to_run_away()
        #If player did not run away or heal, he will attack an enemy
        else:
            """ attack an enemy """
            self.hp_after_potion = self.current_health
            if self.state == c.SELECT_ACTION:
                self.notify(c.CLICK2)
                self.state = c.SELECT_ENEMY
		...
		#the following code was for keystroke inputs
                """
